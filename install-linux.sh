...
if ! command -v node &>/dev/null; then
    ...
    if [ "$PKG" = "apt" ]; then
        ...
        echo "  Install Node.js 20+ manually from nodejs.org then re-run."
        exit 1
    fi
fi
...
if [ "$NODE_MAJOR" -lt 18 ]; then
    echo "  Node.js >= 18 required. Please update."
    exit 1
fi
...
if ! command -v git &>/dev/null; then
    ...
    elif [ "$PKG" = "pacman" ]; then sudo pacman -S --noconfirm git; fi
fi
